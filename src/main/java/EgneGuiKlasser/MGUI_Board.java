...
/**
 * Kopi af Maven:diplomitdtu:matadorgui:3.1.7.gui_fields.GUI_Board som er tilrettet
 */
 ...
 * @author Ronnie
 */
public final class MGUI_Board extends javax.swing.JFrame implements Observer
{
    ...
    public MGUI_Player[] playerList = new MGUI_Player[MAX_PLAYER_COUNT];
    ...
    private MGUI_Field[] fields = null;
    private int die1x = 1, die1y = 1, die2x = 1, die2y = 1;

    private ImageIcon[] hotelIcons = new ImageIcon[6]; // TSR
...
//    }
    ...
    public MGUI_Board(MGUI_Field[] fields, Color backGroundColor) {
        ...
        for(MGUI_Field field : fields){
            if (field!=null)
                field.addMouseListener(new MFieldMouseListener(field, playerList));
        }
...
//      this.factory.setSize(this.base, 11 * 100, 11 * 63);
 ...
 // TSR               this.factory.setSize(label, MGUI_Field.FIELDWIDTH, MGUI_Field.FIELDHEIGHT);
     ...
     * @return fields ref
     */
    public MGUI_Field[] getFields() { return fields; }
    /**
     ...
     */
    public MGUI_Player getPlayer(String name) {
        for(MGUI_Player p : playerList) {
    ...
    // TSR
    private void makeHouse1()
    {
        try {
            String path = Attrs.getImagePath("GUI_Board.House1");
            BufferedImage image = ImageIO.read(getClass().getResource(path));
            for(int value = 0; value < 6; value++) {
                int x = 0;
                int y = 55 * value;
    ...
    private void makeHotel()
    {
        ...
        try {
            ...
            for(int value = 0; value < 6; value++) {
                int x = 0;
                int y = 5 * value;
                this.hotelIcons[value] = new ImageIcon(image.getSubimage(x, y, 5, 5));
            }
        } catch(IOException ex) {
            ex.printStackTrace();
        }
